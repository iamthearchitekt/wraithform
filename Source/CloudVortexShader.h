...
const static char *cloudVortexFragmentShader = R"glsl(
    ...
    void main() {
        ...
        vec2 p = (gl_FragCoord.xy - center) / u_resolution.y;
        ...
        // Pure theme color alignment — clean, monochromatic volumetric depth
        float luma = dot(col, vec3(0.299, 0.587, 0.114));
        vec3 themeCloud = mix(u_glowColor * 0.12, u_glowColor * 1.5, luma);
        col = themeCloud * luma * 2.2;

        // Dark atmospheric background (Darkened by 30%)
        col *= 0.14;
